...
// Function to display products by category and allow the user to select product by name, enter price, and quantity
void searchProductByCategoryAndName(const vector<Product>& products, const string& category, int consoleWidth) {
    vector<Product> filteredProducts;
    copy_if(products.begin(), products.end(), back_inserter(filteredProducts),
            [&category](const Product& p) { return p.category == category; });
    ...
    auto it = find_if(filteredProducts.begin(), filteredProducts.end(),
                      [&productName](const Product& p) { return p.name == productName; });

    if (it != filteredProducts.end()) {
        // Get price and quantity from the user
        float price;
        int quantity;
        cout << "Enter the price for " << it->name << ": ";
        cin >> price;
        cout << "Enter the quantity for " << it->name << ": ";
        cin >> quantity;
        ...
        // Display the result
        string result = "Product: " + it->name + " | Price: $" + formatPrice(price) +
                        " | Quantity: " + to_string(quantity) + " | Total Cost: $" + formatPrice(totalCost);
        printCenteredWithTabs(" ____________________________________________________________________________", consoleWidth);
        printCenteredWithTabs(result, consoleWidth);
    } else {
...
int main() {
    ...
    while (true) {
        ...
        if (command == 'n') {
            ...
            getline(cin, category);
            system("cls");
            searchProductByCategoryAndName(products, category, consoleWidth);
            system("pause");
        } else if (command == 'q') {
            break;
        } else {
